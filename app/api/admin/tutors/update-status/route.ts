...
export async function POST(req: Request) {
  try {
    ...
    /* ================= 🔥 FIX: UPDATE USER BASED ON STATUS ================= */
    ...
    // ❌ REJECTED → DO NOTHING to User (IMPORTANT)
    ...
    if (status === "APPROVED") {
      ...
      <div style="font-family:Arial;line-height:1.6;color:#333">
        <h2 style="color:#2e7d32">Congratulations ${tutor.name}!</h2>
        <p>Your tutor application has been <b>approved</b>.</p>
        <p>You can now login and start teaching students.</p>
      </div>
      `;
    }
    ...
    if (status === "REJECTED") {
      ...
      <div style="font-family:Arial">
        ...
        <p>Hello ${tutor.name},</p>
        <p>Your tutor application was rejected.</p>
        <p><b>Reason:</b> ${reason}</p>
      </div>
      `;
    }
